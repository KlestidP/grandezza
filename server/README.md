...
- A real Analytics Collector — there's no real traffic locally to collect.
- Auth, CI/CD, Docker, and a test framework.
...
```
POST /api/leads/import                     seed/import leads
POST /api/leads/score-all                   rule-based scoring
POST /api/leads/:id/outreach                Outreach AI Agent → drafts email + letter
POST /api/campaigns/:id/send                queues the send jobs
POST /api/webhooks/email  (or /mail)         simulate a reply
POST /api/campaigns/:id/close               deal closed → provisions a Client
POST /api/clients/:id/onboarding             intake
POST /api/clients/:id/site                    Site Generation Agent → writes a real HTML file
POST /api/sites/:id/deploy                     mock deploy → servable at /sites/:slug.html
POST /api/clients/:id/marketing-posts          Ongoing Marketing Agent
GET  /api/dashboard                            aggregate view of the whole pipeline
```
