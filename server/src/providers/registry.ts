...
export const deployProvider: DeployProvider = mockDeploy;

export function logProviderModes() {
  logger.info(
    `[providers] LLM=${providerModes.llm} EMAIL=${providerModes.email} MAIL=${providerModes.mail} DEPLOY=${providerModes.deploy}`,
  );
}
